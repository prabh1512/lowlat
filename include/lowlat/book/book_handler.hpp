...
struct BookHandler {
    ...
    void operator()(const itch::AddOrder& msg) {
        Order o;
        o.stock    = msg.stock_locate.get();
        o.order_id = msg.order_ref.get();
        o.shares   = msg.shares.get();
        o.price    = msg.price.get();
        o.side     = (msg.buy_sell == 'B') ? Side::Bid : Side::Ask;
        o.prev     = NIL;
        o.next     = NIL;
    ...
    void operator()(const itch::OrderReplace& msg) {
        std::uint64_t t0 = core::rdtsc();
        book.ReplaceOrder(
            msg.original_order_ref.get(),
            msg.new_order_ref.get(),
            msg.price.get(),
            msg.shares.get()
        );
        std::uint64_t t1 = core::rdtsc();
