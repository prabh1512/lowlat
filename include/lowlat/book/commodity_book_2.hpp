...
struct CommodityBookV2 {
    ...
    void Add(Order order, std::uint32_t idx, OrderPool& pool) {
        ...
        if constexpr (S == Side::Bid) {
            ...
            Add(AskLevels, AskHT, std::less<Price>(), order.price, order.shares, idx, pool);
        }
    ...
    bool Reduce(Price price, Shares delta, std::uint32_t idx, OrderPool& pool) {
        ...
        bool removed;
        if constexpr (S == Side::Bid) {
            removed = Reduce(BidLevels, BidHT, std::greater<Price>(), price, delta, idx, pool);
        } else {
            removed = Reduce(AskLevels, AskHT, std::less<Price>(), price, delta, idx, pool);
        }
        std::uint64_t t1 = core::rdtsc();
